import { InputStream } from "fthtml/lib/lexer/input-stream";
import { LEX_MODE, char, tokenposition, TokenPosition } from "fthtml/lib/lexer/types";
import { ftHTMLInvalidCharError, ftHTMLexerError } from "fthtml/lib/utils/exceptions";
import { default as ftHTMLGrammar } from "fthtml/lib/lexer/grammar";
import Token, { TOKEN_TYPE as TT, Tokenable, token, TokenStream, getTokenTypeForIdentifier } from "./token";

export class ftHTMLLSLexer {

    static TokenStream = function (input: InputStream): TokenStream {
        let ELANG_MODE = null;
        let LM: LEX_MODE | null = LEX_MODE.NULL;
        ...
        function readString(): token {
            ...
            return Token(TT.STRING, `${delimiter}${readEscaped(delimiter)}${delimiter}`, pos);
        }
