    ...
    private async exportftHTML() {
        ...
        if (ws) {
            ...
            try {
                ...
                if (this.configs.shell)
                    // @ts-ignore
                    options['shell'] = this.configs.shell

                let { stderr, stdout } = await exec(`node "${path.extensionPath}/node_modules/fthtml/cli/bin/fthtml" convert`, options);
